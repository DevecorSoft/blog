...
> 与其从一个工作流程复制和粘贴到另一个工作流程，不如使用可复用工作流
> ---[Github Actions](https://docs.github.com/en/actions/using-workflows/reusing-workflows)

显然，这是用来提升流水线代码复用的一个特性。其实在Github Actions中，有三种复用方式：

1. 脚本复用 - 你可选择任何一种脚本语言
2. 第三方Actions - 在github marketplace中选用
3. 可复用工作流

脚本复用是粒度非常小的一种复用方式，严格来讲，与流水线平台并没有太大的关系，github actions只负责运行而已。比如使用javascript便携一个脚本从`package.json`中读取版本号，抛出一个异常就能让流水线失败，这通常在任何一个流水线工具里都是可用的。

若你发现编写的小脚本具备通用性，便能选择把它封装成第三方Actions供大众使用。当然这意味着我们通常会出于简单和效率的考量选择引入一些第三方的Actions到自己的工作流

更近一步，你发现多个工作流总是具有相似的作业和步骤，于是你把他们抽取出来，便成了这里所说的可复用工作流

值得一提的是，可复用工作流是作为一个工作流（workflow）级别的组件定义的，但调用时却是作为一个单独的作业（job）运行。既然作为一个作业，是需要被分配独立的运行器，这通常会造成10s左右的等待期（也许更久）。实践中需要一定的权衡，避免高射炮打蚊子的情况。
