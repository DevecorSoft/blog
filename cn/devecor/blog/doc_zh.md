...
然而却很少能见到关于如何正确组织技术文档的实践及方法论。团队工作中，编写文档仍面临挑战。

##### 挑战一： 仓促的开始和结束

编写技术文档这个任务看起来总是：优先级不够高，特别花时间并且没有立竿见影的正反馈！于是文档编写被一推再推，直至在某些时刻不得不做，比如新人要上项目了，我的开源产品要发布了，才震惊的发现我竟没有文档。文档最后被随意的编写，以至于完成后就被彻底忽视。随着系统的演进，这些文档慢慢脱节，变成谎言！这个论断乍一看如此的荒谬，可是却在我身边常常发生。

##### 挑战二： 混乱的结构

如同代码编写一样，混乱的结构是相当致命的。我们能使用类似于[technical-writing-template](https://bolajiayodeji.github.io/technical-writing-template/)这样，基于模板约定的方式使得单篇文章的质量达到一定的水准。在复杂的软件系统中，高质量的单篇显然是杯水车薪的，解决办法也是显然的： 结构化。事实上，众多优秀的软件产品它们基本都具备恰到好处的文档，以结构化的方式使得初学者和长期用户读起来都毫不费力。我以为，未能使文档有效结构化的原因有以下几点：
...
2. 缺少对抗混乱的模式。不同于软件编写，我们有**架构风格**这样深入人心的默认约定。甚至有[C4 model](https://c4model.com/)来可视化软件架构，帮助团队保持一致的认知，并使架构有序的演进。文档的编写除了本文将要介绍的[文档象限](https://documentation.divio.com/)之外，并未发现任何一种有较大影响力的结构化文档组织方式。

----------------------------------------------------------------------------

至此，笔者肯定了结构化文档具备显而易见的可读性，以及编写结构化文档面临着的挑战。结构化文档之外似乎还存在另一种文档组织方式：**图谱化**，并且初具影响力。

很多时候，为了保持文章的简洁和内聚，我喜欢使用链接文字将一个相关概念指向别处。一旦顺着链接深入几层，就会发现文档所承载的知识很快组成一张大网。知识图谱一词简直恰如其分。自2012年谷歌知识图谱发布以来，知识图谱的主要用武之地仍在搜索引擎，文献检索领域。有诸如[logseq](https://logseq.com/)这样的产品另辟蹊径，强化知识之间的链接，以图谱化的方式组织文档。在使用logseq的过程中，我发现这种方式更契合人类在大脑中构建的知识模型，有利于深刻又全面的理解问题。这与卢曼的《卡片笔记写作法》有异曲同工之妙。

笔者以为，图谱化的文档组织方式在团队中更适合知识的生产和管理，因为其主题和边界都是不明确的。而结构化的文档更适合对外输出，将之提供给你的用户。

[文档象限](https://documentation.divio.com/)提供一种模式确保你编写正确的结构化文档。划分象限的两条轴描述了文档所具有的属性，横轴描述文档的使用场景是偏于工作的还是偏于学习的，纵轴描述文档是偏于理论的还是偏于实践的。 此处或许有必要明确引用一下文档四象限的内涵:
